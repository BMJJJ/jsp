...
/*
  자바 파일업로드시...(컨트롤러에 아래 어노테이션에 환경설정처리한다.)
	@MultipartConfig(
			...
			maxFileSize = 1024 * 1024 * 10, 올릴수 있는 용량
			maxRequestSize = 1024 * 1024 * 10 * 10 대충 총용량 정해줌 전체의
	)
 */
...
public class FileUpload1OkCommand implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		// 파일 업로드 처리...(객체 생성시 파일이 자동으로 업로드 된다.) multipartRequest여기다가 정보를 일임시켜놔서 request(이렇게 부르면 껍데기만 가져온것) 말고 multipartRequest사용해야 한다
