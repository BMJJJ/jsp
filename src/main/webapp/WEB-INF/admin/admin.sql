...
select * from review v, reviewReply r where v.partIdx=8 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
select * from review v left join reviewReply r on v.partIdx=8 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
select * from (select * from review where partIdx=8) as v left join reviewReply r on v.partIdx=8 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
